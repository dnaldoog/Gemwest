...
Dictionary<String^, Double>^ diaCut = gcnew Dictionary<String^,Double>;
...
this->m_diaCutNames = diaCutNames;
	...
	void CDiamondCut::setCut(Dictionary<String^, Double>^ arr) {
		this->m_diaCut = arr;

	}

	Decimal CDiamondCut::getCutFactor(String^ key) {
		return System::Convert::ToDecimal(this->m_diaCut[key]);
	}
